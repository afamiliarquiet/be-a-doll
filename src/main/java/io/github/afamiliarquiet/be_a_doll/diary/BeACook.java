	...
	public static class EssenceArtistryRecipe extends SpecialCraftingRecipe {
		...
		public boolean matches(CraftingRecipeInput input, World world) {
			if (input.getStackCount() != 2) {
				...
				for (int i = 0; i < input.getSize(); i++) {
					...
					if (!current.isEmpty()) {
						if (current.isIn(BeAResearcher.DOLLCRAFT_ITEMS) || current.isIn(BeAResearcher.UNDOLLING_ITEMS)) {
		...
		public ItemStack craft(CraftingRecipeInput input, RegistryWrapper.WrapperLookup registries) {
			...
			for (int i = 0; i < input.getSize(); i++) {
				...
				if (!current.isEmpty()) {
					...
					} else if (current.isIn(BeAResearcher.UNDOLLING_ITEMS)) {
						if (dollVariant != null) {
							return ItemStack.EMPTY;
						}
		...
		public DefaultedList<ItemStack> getRemainder(CraftingRecipeInput input) {
			...
			for (int i = 0; i < remainders.size(); i++) {
				...
				} else if (current.isIn(BeAResearcher.DOLLCRAFT_ITEMS) || current.isIn(BeAResearcher.UNDOLLING_ITEMS)) {
