...
import ink.iridith.be_a_doll.mixin.synthetic_treats.FoxTrustInvoker;
	...
	public @NotNull InteractionResult interactLivingEntity(@NotNull ItemStack stack, @NotNull Player user, @NotNull LivingEntity entity, @NotNull InteractionHand hand) {
		if (entity instanceof Player doll && BeAMaid.isDoll(doll)) {
			if (doll.startRiding(user, false)) {
				user.playSound(BeABirdwatcher.RAVEN_CHIRP.get(), 1f, 1f);
	...
	public InteractionResult useToTryRiding(ItemStack stack, Player user, Entity entity, InteractionHand hand) {
		if (BeAMaid.isDoll(user)) {
			...
			if (entity instanceof OwnableEntity tameable) {
				...
				if (ownerRef != null && ownerRef.getUUID().equals(user.getUUID()) && entity.getBbWidth() > user.getBbWidth()) {
					shouldRide = true;
				}
			} else if (entity instanceof Fox foxesAreSoCool && ((FoxTrustInvoker)foxesAreSoCool).invokeTrusts(user.getUUID())) {
				shouldRide = true;
			}
...
//			user.removeAllPassengers();
			...
			if (!world.isClientSide()
				...
				serverPlayerEntity.changeDimension(new DimensionTransition((ServerLevel) serverPlayerEntity.level(), pos, Vec3.ZERO, user.getYRot() + 180, user.getXRot() * -1, DimensionTransition.DO_NOTHING));
			}
			user.playSound(BeABirdwatcher.RAVEN_CRY.get(), 1f, 1f);
